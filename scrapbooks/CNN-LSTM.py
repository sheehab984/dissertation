...
df = pd.DataFrame.from_dict(
    {"timestamp": df["timestamp"], "ask": df["ask"], "bid": df["bid"]}
)
...
input_shape = (5, 1)  # (timestamps, features)
...
model = Sequential(
    [
        ...
        Dropout(0.2),
        # Replace N with the number of classes
        # you have in case of classification, or 1 for regression.
        Dense(
            1, activation="linear"
...
earlystop = EarlyStopping(
    ...
    verbose=1,
    mode="auto",
)  # Direction that your monitor value should go
...
# Compile the model
model.compile(
    optimizer=Adam(learning_rate=0.001), loss="mean_squared_error", metrics=["accuracy"]
)
...
df = pd.DataFrame(
    {"trainPredict": testPredict[:, 0].flatten(), "trainY": testY.flatten()}
)
